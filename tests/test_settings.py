...
def test_the_sketch_panel_is_off_unless_the_hub_asks_for_it(monkeypatch):
    """An unset variable means NO, and that is the whole point of the field.

    The sketch panel is a feature still being tried out, so the question this
    ...
    """
    monkeypatch.setenv("EDIT_TOKEN", "s3cret")
    monkeypatch.delenv("SKETCH_PANEL", raising=False)
    assert Settings(_env_file=None).sketch_panel is False
    # And it is reachable, which is the other half: a flag nothing can turn on
    # is a feature nobody can try.
    monkeypatch.setenv("SKETCH_PANEL", "true")
    assert Settings(_env_file=None).sketch_panel is True
