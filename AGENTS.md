...
- **Вьювер форкнут в `viewer/`, three вынесен из бандла** (issue #14).
  `static/_v/three-cad-viewer.*` — не «вендоренный npm-артефакт», а НАШ выход:
  правишь `viewer/src/` → `make viewer` → коммитишь исходник и сборку одним
  коммитом. Рядом лежат `three.module.js` и `three.core.js` — вот они как раз
  npm-овские, копируются без изменений. Импорт-карты на странице нет и быть не
  может: CSP хаба режет инлайновый скрипт, путь к three вписан в бандл на сборке.
