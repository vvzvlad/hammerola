...
Это самая ценная находка. В нашем `viewer.js` измерения выключены с комментарием
«needs a backend we do not run (SPEC 2.4)» — **это устарело**. В версии 5.0.1 есть
...
**Наш `static/_v/viewer.js` это уже решает** — см. `keepSectionCut()` (:938–950). При
переносе логику не выбрасывать.
