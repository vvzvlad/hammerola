"""`make cad-test` names four tests by node id, and node ids rot.

The target is the ONLY thing that runs the four tests the CI container cannot
...
This does NOT run the four; running them needs the kernel and that is the
...
def _declared_node_ids() -> list[str]:
    ...
    assert match, "the Makefile no longer declares CAD_TESTS; `make cad-test` " \
                  "is what runs the four tests CI skips (issue #27)"
    body = match.group(1).replace("\\\n", " ")
    return body.split()


def test_the_target_still_names_four_tests():
    """Four, and the number is deliberate -- see issue #27 for which four.

    A fifth that arrives without a decision, or a fourth that quietly leaves,
    both change what "we catch it by hand" covers, and neither should happen
    without somebody reading this line.
    """
    assert len(_declared_node_ids()) == 4
