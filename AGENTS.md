...
- [ ] **Вне нумерации 8A.2, но раньше всего:** завести репозиторий
      `projects/hammerola` в Gitea, добавить `origin`, положить в секреты репозитория
      `REGISTRY_TOKEN` и спушить. Сейчас ремоутов нет вовсе — проект существует в
      одном экземпляре на диске, а первый же пуш в `main` упадёт на логине в реестр.
      Путь образа в compose (`gitea.vvzvlad.xyz/projects/hammerola`) и путь, который
      CI вычисляет из имени репозитория, совпадут только при этом имени.
