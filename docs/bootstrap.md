...
## ⚠️ Переезд закончен: все девять шагов закрыты, осталось снести эту секцию
...
**Открытых шагов плана не осталось: шаг 8 закрыт 2026-09-14.** Чеклист ниже
ведётся по шагам плана и он же авторитет: прежде чем поверить любому абзацу этой
секции, что чего-то «ещё нет», сверься с ним. В чеклисте открыт ровно один
пункт, и он про сам этот файл — снести секцию и оставить постоянную часть; это
решение человека, а не механическая уборка, поэтому оно ждёт его слова.
...
- [x] **Шаг 5. Асинхронный приём** — 202, идентификатор задачи, эндпоинт статуса и
      ...
      `MAX_CONCURRENT_PUBLISHES`, но это совпадение двух независимых решений, а
      не связь, и замер из них только один — сборочный;
...
- [x] ~~**Шаг 8. Сравнение ревизий**~~ — сделано 2026-09-14, двумя резами
      (issue #10 «Сравнение двух ревизий», SPEC 8A.2). Движок считает General
      Fuse по паре одноимённых деталей и не показывает результат, которому
      нельзя верить; `hammerola diff --material` печатает пофайлово, сколько
      материала прибавилось и убыло; а страница
      `/project/<pid>/<a>/compare/<b>/` рисует обе ревизии полупрозрачными
      одним нейтральным цветом и саму разностную геометрию — ярким, в одном
      окне, с легендой и списком затронутых деталей. Сравнение — вещь под
      токеном: считается задачей в очереди сборок, кэш лежит вне каталогов
      сборок. Слово `compare` стоит ЧЕТВЁРТЫМ сегментом именно поэтому: вторым
      оно потребовало бы зарезервировать валидное сегодня имя сборки.
